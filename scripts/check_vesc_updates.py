...
class VESCUpdateChecker:
    def __init__(self):
        ...
        if not self.github_token:
            print("Warning: GITHUB_TOKEN not set. Using unauthenticated requests (60 req/hour limit)")
            print("For higher limits, set GITHUB_TOKEN in .env file or environment variable")
        ...
        for i, commit in enumerate(commits):
            ...
            if commit_details:
                info = {
                    ...
                    'additions': sum(f.get('additions', 0) for f in commit_details.get('files', [])),
                    'deletions': sum(f.get('deletions', 0) for f in commit_details.get('files', [])),
                }
                commit_info.append(info)
                print(f"      Found {len(info['files_changed'])} changed files")
            else:
                print(f"      Warning: Could not fetch details for commit {commit['sha'][:8]}")
...
"""
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert software developer specializing in embedded systems and communication protocols."},
...
def main():
    parser = argparse.ArgumentParser(description="Check VESC firmware updates for SDK compatibility")
    parser.add_argument("--force", action="store_true", help="Force check all recent commits (ignore last checked)")
